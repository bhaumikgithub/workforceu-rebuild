...
import StripeCardField from '@/components/ui/form/StripeCardField';
import {
    emailRegex,
    phoneRegex,
    requiredMsg,
...
interface Plan { id: string; nickname: string; amount: number; currency: string; interval?: string; }
interface Coupon { id: string; name: string; percent_off?: number | null; amount_off?: number | null; }
...
function PublicUserForm() {
    ...
    const onSubmit = async (data: FormData) => {
        ...
        // 2. Validate subscription plan if payment type is credit card
        if (data.payment_type === "CC" && !data.plan_name) {
        ...
        // 3. Handle Stripe payment method if CC
        ...
        try {
            ...
            const payload = {
                ...data,
                paymentMethod: paymentMethodId,         // Stripe backend expects this
                subscription_plan: data.plan_name,      // Stripe price ID
                coupon: data.coupon_code || undefined   // optional coupon
            };

            // 5. Call backend API
            await post("/api/admin/publicUsers", payload);
    ...
    return (
        <form onSubmit={handleSubmit(onSubmit)} className="grid grid-cols-1 md:grid-cols-2 gap-6">
            ...
            <div className="space-y-4">
                ...
                <InputField label="Client ID" placeholder="Client ID" register={register("subdomain_value", {
                    required: requiredMsg("Client ID"),
                    pattern: { value: /^[a-z0-9]+$/, message: "Only lowercase letters and numbers allowed", },
